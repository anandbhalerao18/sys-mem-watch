...
#include <getopt.h>   // For handling command line args (though we're not using them much here) never used before 

#define LOG_FILE "logs/log.txt"  // Where we'll store our logs
#define INTERVAL 5               // How often to check memory (in seconds) this is interval

// Gets how much RAM is being used right now (returns percentage)
float get_memory_usage() {
    // Open the system's memory info file of linux 
    FILE *fp = fopen("/proc/meminfo", "r");
    if (!fp) {
        perror("Failed to open /proc/meminfo");  // error handling we call it 
        exit(1);
    }
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        if (usage > 90) {
            printf(" Warning: Memory usage is above 90%%\n");
        }
